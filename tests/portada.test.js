...
const html = await readFile(new URL('../portada.html', import.meta.url), 'utf8');
...
test('Portada exige sesión y limita acceso a los tres usuarios autorizados', () => {
  for (const email of ['keanuavia@gmail.com', 'r.labbe.a@gmail.com', 'admin@aviarockets.cl']) assert.match(js, new RegExp(email.replace('.', '\\.')));
  assert.match(js, /avia_auth_token/);
  assert.match(js, /login\.html\?next=/);
});

test('Portada incluye editor, configuración y exportación PNG', () => {
  assert.match(html, /Configuración de portada/);
  assert.match(html, /Exportar PNG/);
  assert.match(js, /toDataURL\('image\/png'\)/);
  assert.match(js, /JoacoAvia · Grid \+ Topografía/);
});
